...
#include <iostream>
MouseLookEvent::MouseLookEvent(Character* character_, RenderingWindow* window_,
    Camera* camera_) :
	InputEvent("MouseLook"), character(character_), window(window_),
	    camera(camera_), mouseReady(true) {
}
...
bool MouseLookEvent::action(InputEvent::Inputs input, int mouseX, int mouseY) {
	...
	//rotate character so it is facing camera
